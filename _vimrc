...
colorscheme catppuccin  # Load the Novum color scheme.
...
def WaylandCopy(reg: string, regtype: string, lines: list<string>)
    var cmd = 'wl-copy --type text/plain'
    if reg == '*'
        cmd ..= ' --primary'
    endif
...
def WaylandPaste(reg: string): list<any>
    var cmd = 'wl-paste --no-newline'
    if reg == '*'
        cmd ..= ' --primary'
    endif
...
set clipmethod^=osc52,wl_clipboard  # Add OSC 52 escape sequences and wl-clipboard as clipboard transports.
