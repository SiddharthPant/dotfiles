...
Shell configuration lives in `.zshenv`, `zshrc/{macos,arch-i3}/`, `.config/fish/{macos,wsl}/`, and `powershell/`; terminal configuration in `.tmux.conf`, `.config/ghostty/`, and `windows_terminal/`; editor configuration in `.config/{nvim,zed,emacs}/`, `.vimrc` (Unix) and `_vimrc` (Windows), and `vscode/`; and tool configuration in `.gitconfig`, `.config/{starship.toml,gh,jj,herdr,sqlfluff}`, `.config/mise/{macos,wsl}/config.toml`, `.pi/`, and `.claude/statusline.js` (Node Claude Code status line).
...
Copied files (Windows copies and seeded files such as `.pi/agent/settings.json`) can drift from the repo. Before changing a copied source or re-copying it, diff the source against its destination (`git diff --no-index <source> <destination>`) and reconcile:
